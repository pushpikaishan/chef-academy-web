  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const isKitchen = (dept) => {
          const d = (dept || '').trim()
          return d === 'Kitchen' || d === 'Hot & Cold Kitchen'
        }
        setRecipes(Array.isArray(data) ? data.filter(r => isKitchen(r.department)) : [])
      } catch (e) {
    ...
    <div style={{ width: '100%', minHeight: '100vh', background: 'linear-gradient(135deg, #f5f5f5 0%, #e8e8e8 100%)', padding: '60px 20px' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: '#666', marginTop: 8 }}>Showing all recipes in the Kitchen department</p>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
            {recipes.map((item) => (
              <div
                ...
                onClick={() => navigate(`/kitchen/recipes/${item._id}`)}
                role="button"
