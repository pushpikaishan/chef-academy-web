...
exports.createTool = async (req, res) => {
  try {
    const { name, photo, description, category } = req.body;
    if (!name) return res.status(400).json({ error: 'name is required' });
    const tool = await Tool.create({ name, photo, description, category });
...
exports.updateTool = async (req, res) => {
  try {
    ...
    const allowed = ['name', 'photo', 'description', 'category'];
    const updates = {};
